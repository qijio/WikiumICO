...
$(function () {
    ...
    $('#copy-smart-address').on('click', function () {
        ...
        $('#copy-smart-address').attr('title', 'Скопирован!').tooltip('show');
    });
...
function fillHead() {
    $.getJSON('data.json', function (local) {
        ...
        if (now < preIcoDate) {
            ...
            $('#timerHeader').text('До завершения компании:');
        }
        else {
            ...
            $('#timerHeader').text('До нового года:'); 
        }
    ...
    if (ethIn.is(':focus')) {
        ...
        if (Math.ceil(token) < new Number(discountedToken))
            $('[id^="currentDiscount"] > strong')
                .append(" (вы&nbsp;получите:&nbsp;" + discountedToken + "WTK)");
    ...
    window.incomeChart = new Chart(gr.ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderWidth: 4,
                data: [
                    48154700, 39314603, 40994845, 71298443,
                    95644184, 83700133, 88795438, 150371609,
                    157450436, 142215463, 158049827, 229366340],
            }],
        },
        options: {
            responsive: true,
